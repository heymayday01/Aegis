  ...
  return (
    <section id="architecture" className="scroll-mt-20 border-t border-border">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16 sm:py-24">
        <SectionHeading
          ...
          title={
            <>
              ...
              <span className="italic text-muted-foreground">works.</span>
            </>
          }
        ...
        {/* System diagram — hard-edged outer frame, inner cells with their own borders */}
        <div className="mt-8 border border-border bg-card p-4 sm:p-5">
          ...
          <div className="mt-4 pt-4 border-t border-border flex flex-wrap items-center justify-center gap-2 text-[11px] text-muted-foreground">
        ...
        {/* Upgrade callouts — hard-edged, border-based entity-tinted tag */}
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          ...
          <div className="border border-border bg-card p-5 sm:p-6">
            ...
            {/* Honest v1 claim callout — hard-edged with left accent */}
            <div className="mt-4 border border-border border-l-2 border-l-primary bg-background/40 p-3 text-xs">
          ...
          {/* Portfolio narrative — primary-tinted */}
          <div className="border border-primary/30 bg-primary/5 p-5 sm:p-6">
            <div className="flex items-center gap-2 mb-3">
              <Lightbulb className="size-4 text-primary" />
              <h3 className="aegis-eyebrow text-muted-foreground">Why this is portfolio-worthy</h3>
            </div>
            ...
            {/* Hard-edged badges (not shadcn Badge) */}
            <div className="mt-4 flex flex-wrap gap-1.5">
              <span className="inline-flex items-center border border-primary/30 text-primary px-2 py-0.5 text-[11px] aegis-mono">
                $0 infra
              </span>
              <span className="inline-flex items-center border border-primary/30 text-primary px-2 py-0.5 text-[11px] aegis-mono">
                Open-source
...
function DiagramColumn({
  ...
  return (
    <div className="border border-border bg-card p-3 flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-px bg-border border border-border flex-1">
        {items.map((it) => (
          <div key={it.title} className="bg-card p-2">
            <div className="text-xs font-medium">{it.title}</div>
            <div className="text-[10px] text-muted-foreground aegis-mono">{it.sub}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="border border-primary/40 bg-primary/5 p-3 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] aegis-mono border border-primary/30 text-primary px-1.5 py-0.5">
          @aegis/core
        </span>
      </div>
      <div className="grid grid-cols-2 gap-px bg-border border border-border">
        {[
          ...
          <div key={m.name} className="bg-card p-2">
            <div className="text-xs font-medium">{m.name}</div>
            <div className="text-[10px] text-muted-foreground aegis-mono">{m.sub}</div>
          </div>
        ))}
      </div>
    </div>
...
function UpgradeCard({
  ...
  return (
    <div className={cn('border border-border bg-card p-5 sm:p-6 flex flex-col gap-3', tone)}>
      {/* Border-based colored tag — uses .entity-XXX (sets --ec) + .entity-chip for the
          bg/border/color mix. Hard-edged, no soft fill. */}
      <span className="entity-chip inline-flex items-center gap-1.5 px-2 py-1 self-start w-fit">
